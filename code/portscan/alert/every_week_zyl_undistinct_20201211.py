...
def bar_chart(conn):
    ...
    sql_bar = "select bline,count(bline) as total from oms_portscan where  cdate BETWEEN %s AND %s GROUP BY bline HAVING total>=1 order by total desc"
    try:
        val_bar = (first,end)
        # 执行sql语句
    ...
    for row in res_week:
        ...
        print "num=", num
        # 业务线仅取最后字符串
        bline_name = bline.split('-')[-1]
        lists[0].append(bline_name)
        lists[1].append(num)

        #折线图
        sql_zx = "SELECT cdate,COUNT(*) as count from oms_portscan where  bline=%s and cdate BETWEEN %s AND %s GROUP BY cdate"
        print "sql_zx=", sql_zx
        try:
        ...
        if len(zhexian_date) >= 1:
            for row in zhexian_date:
                #print "row=",row
                cdate = row.get('cdate').strftime('%Y-%m-%d')
                num = row.get('count')
            ...
            layout = go.Layout(title=dict(text=bline_name + u"本周敏感服务开放情况", font=dict(family='sans-serif',size=20, color='#392f41'), x=0.5),
            ...
            if bline_name == "步":
                ...
                fb_name = bline_name + str(time_unix)[0:8] + ".png"
                file_html_zxn = bline_name + str(time_unix)[0:8] + ".html"
    ...
    if len(x) >=1:
        ...
        for i in range(len(x)):
            ...
            send_ding(conn, bline ,first,end,file_name,fb_name,file_html_zzn,file_html_zxn)
...
#发送消息
def send_ding(conn,bline,first,end,file_name,fb_name,file_html_zzn,file_html_zxn):
    cursor = conn.cursor()
    bline = '%' + bline
    print "bline=", bline
    sql_address = "select bline,yachurl,yachsecret from oms_portscan,authorization_group where oms_portscan.group_id=authorization_group.`id` and bline like %s and cdate BETWEEN %s  AND %s GROUP BY bline,yachurl,yachsecret"
    vul = (bline,first,end)
    try:
        # 执行sql语句
    ...
    for m in res_address:
        address = m.get('yachurl').encode("utf-8")
        ...
        print "开始发送消息"
        title = "本周敏感服务开放情况"
        text = "### **本周敏感服务开放情况:** \n" + "![screenshot](https://bal.com/image/every_week/July/" + file_name + ")\n" + "##### [查看柱状图原图](https://b.com/image/every_week/" + file_html_zzn + ") \n"+ "![screenshot](https://b.com/image/every_week/July/" + fb_name + ")\n" + "##### " + this_week_start_all + "至" + this_week_end_all + " [查看折线图原图](https://bl.com/image/every_week/" + file_html_zxn + ") \n"+ "\n"

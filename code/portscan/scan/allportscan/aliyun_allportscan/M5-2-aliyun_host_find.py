...
def get_iplist():
    ...
    if int(now_num) < 5000:
        dingmsg(now_num,num)
           # sys.exit(0)
    try:
        curs.execute("select publicIpAddress,eipAddress from cms_hosts where updateTime like '%s' and (publicIpAddress !='' OR eipAddress !='') and host_position!='idc'" % (CDATES+'%',))
    except Exception,e:
        print e
    all_result = curs.fetchall()
    ip_num = len(all_result)/6
    print 'all ip num is',len(all_result)
    for item in all_result[ip_num:]:
#        print item
        if item[0]:
            ALLIPS += item[0] + "\n"
        elif item[-1]:
...
def dingmsg(message,msg):

    webhook = 'https://yalou.com/robot/send?access_token=OHpuTUNIKw'
    secert = 'SEC2320a79e9'
    # 初始化机器人小丁
    xiaoding = DingtalkChatbot(webhook,secert)
    text = "阿里云ecs记录更新异常，今日更新总数为"+str(message)+"    昨日总数为"+str(msg)
    xiaoding.send_text(text, at_mobiles=['1826'])
    print "发送消息成功"
